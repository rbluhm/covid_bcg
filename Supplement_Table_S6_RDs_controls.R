...
# rows
dvs <- rep("lcp_total", 7) 
dv_labs <- c("Panel A. No controls", 
             "Panel B. Population density", 
             "Panel C. Disposable income p.c.", 
             "Panel D. Population density and disposable income p.c.",
             "Panel E. Percent younger 35 and percent older than 60",
             "Panel F. Days since first case",
             "Panel G. All of the above")
add_ctrls1 <- c(NA,"ldisp",  "lpopdens", "ldisp", "percent_age_under35", "elapsed_2020_04_26","all") 
add_ctrls2 <- c(NA, NA, NA, "lpopdens", "percent_age_over60", NA, "all") 
...
full.tbl <- foreach(r = 1:length(dvs), .combine = rbind) %:%
              foreach(c = 1:length(hs), .combine = cbind) %do% {
                ...
                if(add_ctrls1[r]!="all" & !is.na(add_ctrls1[r])) {
                  cov.c.df <- cbind(cov.df, bcg.df[add_ctrls1[r]])
                } else if(add_ctrls2[r]!="all" & !is.na(add_ctrls2[r])) {
                  cov.c.df <- cbind(cov.c.df, bcg.df[add_ctrls2[r]])
                } else if(add_ctrls1[r]=="all" & !is.na(add_ctrls1[r])) {
                  all.c.v <- c("ldisp",  "lpopdens", "percent_age_under35", "percent_age_over60", "elapsed_2020_04_26") 
                  cov.c.df <- cbind(cov.df, bcg.df[all.c.v])
                } else {
                  cov.c.df <- cov.df
                }
...
# create tex output
sink("./output/Supplement_Table_S6_RDs_Controls.tex")
latextable(full.tbl)
sink()

#read and modify

fin.tex <- readLines("./output/Supplement_Table_S6_RDs_Controls.tex")
...
fileConn<-file("./output/Supplement_Table_S6_RDs_Controls.tex")
