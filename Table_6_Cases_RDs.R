...
# rows
dvs <- c("lcp_total",  "lcp_c_total", "frac_ddr", "lcp_c_total" ) 

dv_labs <- c("Panel A. Log(1+Cases/Million)", 
             "Panel B. Log(1+Severe Cases/Million)", 
             "Panel C. Fraction of Facebook Connections with East Germany (FFC)", 
             "Panel D. Log(1+Severe Cases/Million), Controlling for FFC" )

add_ctrls <- c(NA,NA,NA, "frac_ddr")
...
# create tex output
sink("./output/Table_6_RD_Cases.tex")
latextable(full.tbl)
sink()

#read and modify

fin.tex <- readLines("./output/Table_6_RD_Cases.tex")
...
fileConn<-file("./output/Table_6_RD_Cases.tex")
